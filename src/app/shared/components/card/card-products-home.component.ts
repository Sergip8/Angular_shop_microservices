...
@Component({
    ...
    <div class="my-5 ">
        ...
        <div class="flex gap-[1px] items-baseline ">
                @for (item of data; track $index) {
                <a [routerLink]="['/p/'+ item.link]" class=" group flex w-full max-w-xs flex-col overflow-hidden border-gray-100 bg-white  dark:bg-gray-400 dark:border-gray-600 ">
                <a class="relative flex h-50 overflow-hidden" href="#">
                  <img class="object-cover items-center" (error)="errorImage($index)" [src]="item.images[0]?.imageUrl ?? ''" alt="product image" />               
                ...
                <div class="mt-4 px-5 pb-5">
                  <a href="#">
                    <h5 class="dark:text-gray-100 text-xl tracking-tight text-slate-900">{{item.name }}</h5>
                  </a>
                  <div class="mt-2 mb-5 flex items-center justify-between">
                    <div class="">
                        <div class="dark:text-gray-300 text-lg text-slate-900 line-through">{{item.previous_price | currency: '$ ':'symbol' :'1.0-0'}}</div>
                        <div class="dark:text-gray-200 text-2xl font-bold text-slate-900">{{item.current_price | currency: '$ ':'symbol' :'1.0-0'}}</div>
                    </div>
                  </div>
